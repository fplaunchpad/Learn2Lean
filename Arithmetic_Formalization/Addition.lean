...
-- The main correctness theorem without carry parameter
-- This is what you actually use when adding two numbers from scratch
-- carryVal false = 0 so the carry term vanishes
theorem verticalAdd_correct' (a b : MultiDigit) :
    toNat (verticalAdd a b false) = toNat a + toNat b := by
  have h := verticalAdd_correct a b false
  simp only [carryVal] at h
  exact h
